...
type Availability = 'desktop' | 'cloud' | 'api';
type Scenario = 'C' | 'R' | 'V' | 'T' | 'E';
...
type ModelEntry = {
  ...
  quality: number;
  score: number;
  // Optional fields referenced in compare view
  format?: string;
  params?: string;
  arch?: string;
  capabilities?: string[];
...
type ModelsProps = {
  onBenchmarkModel?: (name: string) => void;
};
...
function AvailabilityIcons({ items }: { items?: Availability[] }) {
  return (
...
function ScenarioPill({ label, color = 'bg-blue-600' }: { label: string; color?: string }) {
  return (
...
function ChartCard({ title, data, color }: { title: string; data: SeriesPoint[]; color: string }) {
  const now = Date.now();
  const chartData = data.map((d) => ({
    time: d.t,
    value: Number((d.v as number)),
    relMin: (d.t - now) / 60000,
  }));
  const gradientId = `grad-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
  // gradientId kept in case of future gradient usage
  return (
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 6, right: 14, left: 6, bottom: 0 }}>
            ...
            <Tooltip
              ...
              // Recharts formatter signature is flexible; coerce to string tuple
              formatter={(value: any) => [`${Number(value).toFixed(1)}%`, title] as [string, string]}
...
function Models({ onBenchmarkModel }: ModelsProps) {
  ...
  const [cpuSeries, setCpuSeries] = useState<SeriesPoint[]>([]);
  const [ramSeries, setRamSeries] = useState<SeriesPoint[]>([]);
  const [gpuSeries, setGpuSeries] = useState<SeriesPoint[]>([]);
  const [gpuMemorySeries, setGpuMemorySeries] = useState<SeriesPoint[]>([]);
  ...
  const selectedModels = useMemo<ModelEntry[]>(() => {
  ...
  const compareAttrs: {
    ...
    { key: 'tps', label: 'Potential tokens/sec', get: (m) => m.tokensPerSec || (typeof m.speed === 'string' ? m.speed : '—') },
    { key: 'desc', label: 'Description', get: (m) => m.description || '—' },
  ];

  const attrDiffMeta = useMemo<Record<string, { allSame: boolean; vals: string[] }>>(() => {
    const meta: Record<string, { allSame: boolean; vals: string[] }> = {};
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1 ml-2 lg:ml-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 text-sm"
          >
            ...
            <div className="min-w-[720px]">
              <div className="grid" style={{ gridTemplateColumns: `240px repeat(${selectedModels.length}, minmax(220px, 1fr))` }}>
                ...
                {compareAttrs
                  ...
                  .map((row) => {
                    const meta = attrDiffMeta[row.key] || { allSame: false, vals: [] as string[] };
                    const labelBadge = (
          ...
          <div className="mt-4 bg-white border border-gray-200 rounded-xl overflow-auto max-w-full">
            ...
            <div className="min-w-[720px]">
              <div className="grid" style={{ gridTemplateColumns: `240px repeat(${selectedModels.length}, minmax(220px, 1fr))` }}>
                {(() => {
                  const toKey = (v: unknown) => (v === null || v === undefined ? '—' : String(v));
                  const perfRows: { key: string; label: string; get: (m: ModelEntry) => unknown }[] = [
