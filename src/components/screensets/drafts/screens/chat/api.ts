...
class ChatAPI {
  ...
  async getChatThreads(): Promise<ChatTitle[]> {
    ...
    const filteredThreads = this._chatThreadsData.threads
      .filter(thread => !thread.is_deleted && thread.is_active)
  ...
  async createChatThread(title?: string): Promise<ChatTitle> {
    ...
    // Initialize messages for this thread with welcome message
    const welcomeMessageId = generateUUID();
    const welcomeMessage: ChatMessage = {
      ...
      chars_per_sec: 45.2,
      completed: true,
      content: "Hello! I'm your AI assistant. How can I help you today? Feel free to ask me about:\n\n• **React & Frontend Development**\n• **API Design & Backend Development** \n• **JavaScript & Programming Concepts**\n• **Database Design & Optimization**\n• **Performance & Best Practices**\n\nWhat would you like to explore?",
      created_at: now + 100, // Slightly after thread creation
      error: null,
      finish_reason: "stop",
      full_content_length: 321,
      id: welcomeMessageId,
    ...
    // Update thread stats to include the welcome message
    newThread.size_chars = welcomeMessage.content.length;
    newThread.size_tokens = Math.ceil(welcomeMessage.content.length / 4);
    newThread.last_msg_at = now + 100;

    console.log('🆕 New chat thread created successfully with welcome message:', {
      threadId,
      title: defaultTitle,
      welcomeMessageId,
      messageCount: 1
