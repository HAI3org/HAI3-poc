...
// IMPORTANT: Derive the current screenset id dynamically to avoid hardcoding
const getScreensetKey = (): string => {
  try {
    const k = (window as any).__HS_CURRENT_SCREENSET__;
    return typeof k === 'string' && k.length > 0 ? k : 'drafts';
  } catch {
    return 'drafts';
  }
};

export const SCREENSET_KEY = getScreensetKey();
...
export const AVAILABLE_CONTEXTS = [
  ...
  { id: '2', name: 'Context v2', color: 'bg-black' },
  { id: '3', name: 'My', color: 'bg-blue-600' },
  { id: '4', name: 'Test', color: 'bg-cyan-400' },
  { id: '5', name: 'Work', color: 'bg-purple-400' },
  { id: '6', name: 'Work v2', color: 'bg-yellow-600' },
  { id: '7', name: 'Hobby', color: 'bg-red-500' }
] as const;
