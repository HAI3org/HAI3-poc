...
import { ChevronRight, BookOpen, Wrench, Edit3, X, User, FileText, ChevronDown, Clock } from 'lucide-react';
import ChatHistory from './ChatHistory';
import { useChatThreads, useChatSelection } from './hooks';
...
type PanelKind = 'recent' | 'contexts';

interface ExtendedProps extends ChatScreenRSideBarProps {
  activePanel?: PanelKind;
  setActivePanel?: (p: PanelKind | null) => void;
}

const ChatScreenRSideBar: React.FC<ExtendedProps> = ({
  rightSidebarOpen,
  onToggle,
  onClose,
  showAddInstruction,
  ...
  // Hooks for Recent Chats view
  const { chatThreads } = useChatThreads();
  const { selectedChatId, handleChatSelect } = useChatSelection(SCREENSET_KEY, chatThreads);

  const showContent = useMemo(() => rightSidebarOpen, [rightSidebarOpen]);
  // Fallback state if controller not provided
  const [internalPanel, setInternalPanel] = useState<PanelKind | null>(null);
  const panel = setActivePanel ? (activePanel ?? null) : internalPanel;
  const setPanel = (p: PanelKind | null) => {
    if (setActivePanel) setActivePanel(p);
    else setInternalPanel(p);
  };

  // Temporary chat status map (persisted via ChatScreenWrapper)
  const [tempMap, setTempMap] = useState<Record<string, boolean>>({});
  useEffect(() => {
    try {
    ...
    const handler = (event: Event) => {
      ...
      if (e.detail?.screensetKey === SCREENSET_KEY) {
        setTempMap(prev => ({ ...prev, [e.detail.chatId]: !!e.detail.isTemporary }));
      }
    };
    window.addEventListener('chat-temp-toggle', handler);
    return () => window.removeEventListener('chat-temp-toggle', handler);
  }, []);

  const isChatTemporary = (chatId: string) => !!tempMap[chatId];

  return (
    <>
      {/* Right rail - toggle handle */}
      <div className={`absolute top-0 z-30 w-12 h-full hx-rsidebar-rail flex flex-col items-center gap-2 transition-all duration-300 ${rightSidebarOpen ? 'right-[480px]' : 'right-0'}`}>
        {/* Rail buttons at the very top */}
        <div className="pt-3" />
        <button
          onClick={() => {
            if (rightSidebarOpen && panel === 'recent') {
              // Clicking active button collapses and clears selection
              onClose();
              setPanel(null);
            } else {
              setPanel('recent');
              if (!rightSidebarOpen) onToggle();
            }
          }}
          className={`p-2 rounded-lg hx-rail-btn ${rightSidebarOpen && panel === 'recent' ? 'hx-rail-btn-active' : ''}`}
        ...
        <button
          onClick={() => {
            if (rightSidebarOpen && panel === 'contexts') {
              onClose();
              setPanel(null);
            } else {
              setPanel('contexts');
          ...
          className={`p-2 rounded-lg hx-rail-btn ${rightSidebarOpen && panel === 'contexts' ? 'hx-rail-btn-active' : ''}`}
          title={'Show Contexts'}
        >
          <BookOpen size={16} />
        </button>
      ...
      {/* Right overlay panel */}
      <div className={`absolute top-0 z-30 h-full w-[480px] hx-rsidebar transition-all duration-300 ${rightSidebarOpen ? 'right-0' : '-right-[480px]'}`}>
        <div className="flex h-full flex-col">
          <div className="px-4 py-3 hx-rsidebar-header flex items-center justify-between">
            <h2 className="text-lg font-semibold hx-rsidebar-title">{panel === 'recent' ? 'Recent Chats' : panel === 'contexts' ? 'Contexts' : ''}</h2>
            <div className="flex gap-2">
              <button onClick={() => { onClose(); setPanel(null); }} className="p-1 hx-rail-close-btn rounded transition-colors" title="Close sidebar">
                <X size={16} />
              </button>
            </div>
          </div>
            ...
            {panel === 'recent' ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900">Add Instruction</h3>
                  <button onClick={onCancelAddInstruction} className="p-1 hover:bg-gray-100 rounded transition-colors">
                    <X size={20} className="text-gray-500" />
                  </button>
                </div>

                <p className="text-sm text-gray-600">Select existing or add new items to the chat context.</p>
              ...
              <div>
                <p className="text-sm text-gray-600 mb-4">Select existing or add new items to the chat context.</p>
