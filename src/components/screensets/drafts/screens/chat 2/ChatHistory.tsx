...
const ChatHistory = forwardRef<ChatHistoryRef, ChatHistoryProps>(({
  ...
  // Hydrate titles from localStorage on mount to reflect edits made from Chat header
  useEffect(() => {
    try {
      const stored = localStorage.getItem(`${SCREENSET_KEY}-chat-titles`);
      if (stored) {
        const titleMap = JSON.parse(stored) as Record<string, string>;
        Object.entries(titleMap).forEach(([chatId, title]) => {
  ...
  // Removed inline "New Chat" creation UI to simplify sidebar per design
  ...
  const saveInlineEdit = async (chatId: string): Promise<void> => {
    ...
    try {
      ...
      // Persist to localStorage as the source of truth for UI titles
      try {
        const key = `${SCREENSET_KEY}-chat-titles`;
        const map = JSON.parse(localStorage.getItem(key) || '{}');
        map[chatId] = trimmedTitle;
        localStorage.setItem(key, JSON.stringify(map));
      } catch {}

      // Broadcast final title update so Chat header (portal) syncs immediately
      window.dispatchEvent(new CustomEvent('chat-title-update', {
        detail: { screensetKey: SCREENSET_KEY, chatId, newTitle: trimmedTitle }
      }));
    } catch (error) {
  ...
  return (
    <>
      <div
        className="transition-all duration-200 overflow-hidden hx-secondary-menu flex flex-col h-full flex-shrink-0"
        ...
        {showContent && (
          <>
            ...
            {!loading && !error && (
              <>
                {/* Search (full width) */}
                <div className="p-3 border-b hx-recent-chat-search">
            ...
            >
              ...
              >
                <div className="flex-1 overflow-y-auto chat-list-container px-0">
