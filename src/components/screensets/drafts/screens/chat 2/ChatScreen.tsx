...
import { Send, Bot, Paperclip, X, File, Image, FileText, Copy, Edit3, RotateCcw, ThumbsUp, ThumbsDown, Trash2, ChevronDown, Monitor, Cloud } from 'lucide-react';
...
const ChatScreen = forwardRef<ChatScreenRef, ChatScreenProps>(function ChatScreen(props, ref) {
  ...
  useEffect(() => {
    const handleTitleTyping = (event: Event) => {
      ...
      if (customEvent.detail?.screensetKey === SCREENSET_KEY && customEvent.detail?.chatId === currentChatId) {
  ...
  // Listen for finalized title updates to ensure header reflects saved changes
  useEffect(() => {
    const handleTitleUpdate = (event: Event) => {
      const customEvent = event as CustomEvent;
      if (customEvent.detail?.screensetKey === SCREENSET_KEY && customEvent.detail?.chatId === currentChatId) {
        const newTitle = customEvent.detail.newTitle as string;
  ...
  const handleTitleChange = (newTitle: string) => {
    ...
    window.dispatchEvent(new CustomEvent('chat-title-typing', {
      detail: { screensetKey: SCREENSET_KEY, chatId: currentChatId, newTitle }
    }));
  };
  ...
  const handleTitleSave = () => {
    ...
    window.dispatchEvent(new CustomEvent('chat-title-update', {
      detail: { screensetKey: SCREENSET_KEY, chatId: currentChatId, newTitle: editedTitle.trim() }
    }));
  };
  ...
  // Chat Header Component (acts as the screen title)
  const ChatHeader = () => (
    <div className="flex items-center gap-2">
      {editingTitle ? (
        <input
          ...
          className="text-xl font-semibold bg-transparent border-b-2 border-blue-500 focus:outline-none min-w-0"
          autoFocus
        />
      ) : (
        <span
  ...
  useEffect(() => {
    ...
    const rootNode = document.querySelector('.hx-header') as HTMLElement | null;
    setHeaderRoot(rootNode);
  }, []);
  ...
  useEffect(() => {
    ...
    if (headerContainer) {
      ...
      // Hide the default h1 while Chat 2 is active so only the chat title is shown
      const headerRoot = document.querySelector('.hx-header');
      const defaultH1 = headerRoot?.querySelector('h1') as HTMLElement | null;
      let previousDisplay = '';
      if (defaultH1) {
        previousDisplay = defaultH1.style.display;
      ...
      return () => {
        ...
        // Restore the default h1 when leaving Chat 2
        if (defaultH1) {
          defaultH1.style.display = previousDisplay;
        }
      };
    }
  }, []);
  ...
  return (
    <>
      ...
      {headerRoot && ReactDOM.createPortal(
        <div
        ...
        >
          <button
            ...
            className="hx-btn hx-btn-primary px-3 py-1.5 rounded-lg shadow-sm"
            ...
            >
              <div className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className="flex-1 min-w-0">
                  <div className={`${message.type === 'user' ? 'ml-auto w-[61.2%]' : 'mr-auto w-full'} max-w-none`}>
                    {editingMessage === message.id ? (
                      <div className="space-y-3">
                        <textarea
                          value={editedContent}
                          onChange={(e) => setEditedContent(e.target.value)}
                          className="hx-textarea-primary w-full px-3 py-2 rounded-lg resize-none"
                          rows={3}
                        />
                        <div className="flex gap-2">
                          <button
                            onClick={saveEditedMessage}
                            className="hx-btn hx-btn-primary px-3 py-1 text-sm rounded"
                          >
                            Save
                          </button>
                          <button
                            onClick={editAndResetMessage}
                            className="hx-btn hx-btn-secondary px-3 py-1 text-sm rounded"
                          >
                            Save & Reset
                          </button>
                        </div>
                      ...
                      <div className="space-y-2">
                        {message.type === 'user' ? (
                          <div className={`hx-chat-markdown-outer rounded-xl border hx-bubble-user`}>
                            <div className="hx-chat-markdown-middle rounded-xl">
                              <div className="hx-chat-markdown-inner rounded-xl p-4 prose prose-sm max-w-none">
                  ...
                  {editingMessage !== message.id && (
                    <div className={`flex items-center gap-1 mt-2 ${message.type === 'user' ? 'justify-end' : ''}`}>
                      {/* Copy (always) */}
                      <button
                      ...
                      {message.type === 'user' ? (
                        ...
                        <>
                          {/* Regenerate (assistant/other only) */}
                          <button
                            onClick={() => props.regenerateFromMessage && props.regenerateFromMessage(message.id)}
                            className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors text-gray-500 hover:text-gray-700"
          ...
          <div className="mb-3 text-sm text-gray-600 flex items-center justify-between">
            ...
            <div className="inline-block relative model-dropdown">
              <button
                ...
                className="hx-select px-2 py-1 rounded cursor-pointer"
              >
                {selectedModel}
              </button>

              {modelDropdownOpen && (
                <div className="absolute bottom-full left-0 mb-2 w-80 hx-card rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
                  <div className="p-3 hx-card-header">
                    <h3 className="hx-card-title font-medium">Select Model</h3>
                  </div>
                  <div className="py-1 hx-menu">
                    {availableModels.map((model) => (
                      <button
                        key={model}
                        onClick={() => selectModel(model)}
                        className={`hx-menu-item w-full text-left px-4 py-2 text-sm transition-colors ${
                          selectedModel === model ? 'hx-menu-item-selected' : ''
                        }`}
                      >
                        {model}
                      </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <Switch
                checked={!!(isChatTemporary && isChatTemporary(currentChatId))}
                onChange={() => toggleChatTemporary && toggleChatTemporary(currentChatId)}
            ...
            <div className="mb-3 text-sm">
              ...
              <div className="flex items-center gap-2 mt-2">
                {selectedContexts.map(contextId => {
                  ...
                  return (
                    <div key={contextId} className="flex items-center gap-2 px-3 py-1 hx-card rounded-full text-sm">
                      <div className={`w-3 h-3 ${context.color} rounded`}></div>
                      <span>{context.name}</span>
                      <button
                        onClick={() => removeContext(contextId)}
                        className="hx-btn hx-btn-ghost rounded-full p-0.5"
                      >
                        <X size={12} />
                      </button>
                    </div>
          ...
          {attachedFiles.length > 0 && (
            <div className="mb-4 space-y-2">
              {attachedFiles.map((file) => (
                <div key={file.id} className="flex items-center gap-3 p-3 hx-card rounded-lg border">
                  ...
                  <button
                    ...
                    className="hx-btn-text hover:bg-gray-100 rounded-lg transition-colors text-gray-500 hover:text-gray-700"
                  >
                    <X size={16} />
                  </button>
                </div>
